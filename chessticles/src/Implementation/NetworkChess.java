...
public class  NetworkChess extends Thread {
    ...
    /**
     * this sends data to the other computer
    ...
    /**
     *  this sends and recieves moves from game
     * @param s string sent from computer
     * @return string sent from other computer
     */
    public String sendAndWait(String s) {
        ...
        try {
            ...
            System.out.println("UDP datagram length " + thing.length() + "  from IP " + dp.getAddress() + " received: " + s);
    ...
    String blackfirstSend() {
        ...
        try {
            ...
            ds.receive(dp);                                     // wait for next datagram
            thing = new String(dp.getData(), 0, dp.getLength());        // get contenets as a String
            System.out.println("UDP datagram length " + thing.length() + "  from IP " + dp.getAddress() + " received: " + thing);
            dp.setLength(buffer.length);
        } catch (SocketException se) {
            System.err.println("chat error " + se);
        } catch (IOException se) {
